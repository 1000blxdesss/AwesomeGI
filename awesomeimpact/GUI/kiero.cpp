...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.hInstance = LI(kernel32, GetModuleHandleA)(NULL);
			windowClass.hIcon = NULL;
			windowClass.hCursor = NULL;
			windowClass.hbrBackground = NULL;
			windowClass.lpszMenuName = NULL;
			char clsNameBuf[32]; lstrcpyA(clsNameBuf, XS("DXSetup"));
			windowClass.lpszClassName = clsNameBuf;
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, XS("DirectX"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);
			...
			else if (_renderType == RenderType::D3D10)
			{
				...
				HMODULE libDXGI;
				HMODULE libD3D10;
				if ((libDXGI = LI(kernel32, GetModuleHandleA)(XS("dxgi.dll"))) == NULL || (libD3D10 = LI(kernel32, GetModuleHandleA)(XS("d3d10.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
				...
				if ((CreateDXGIFactory = LI(kernel32, GetProcAddress)(libDXGI, XS("CreateDXGIFactory"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
				...
				if ((D3D10CreateDeviceAndSwapChain = LI(kernel32, GetProcAddress)(libD3D10, XS("D3D10CreateDeviceAndSwapChain"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
			...
			else if (_renderType == RenderType::D3D11)
			{
#if KIERO_INCLUDE_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = LI(kernel32, GetModuleHandleA)(XS("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::ModuleNotFoundError;
				...
				if ((D3D11CreateDeviceAndSwapChain = LI(kernel32, GetProcAddress)(libD3D11, XS("D3D11CreateDeviceAndSwapChain"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
		...
		else
		{
			...
			if (LI(kernel32, GetModuleHandleA)(XS("d3d9.dll")) != NULL)
			{
				type = RenderType::D3D9;
			}
			else if (LI(kernel32, GetModuleHandleA)(XS("d3d10.dll")) != NULL)
			{
				type = RenderType::D3D10;
			}
			else if (LI(kernel32, GetModuleHandleA)(XS("d3d11.dll")) != NULL)
			{
				type = RenderType::D3D11;
			}
			else if (LI(kernel32, GetModuleHandleA)(XS("d3d12.dll")) != NULL)
			{
				type = RenderType::D3D12;
			}
			else if (LI(kernel32, GetModuleHandleA)(XS("opengl32.dll")) != NULL)
			{
				type = RenderType::OpenGL;
			}
			else if (LI(kernel32, GetModuleHandleA)(XS("vulkan-1.dll")) != NULL)
			{
